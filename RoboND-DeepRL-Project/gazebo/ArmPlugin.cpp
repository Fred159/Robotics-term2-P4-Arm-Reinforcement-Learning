 ...
 * Modified again by - Ming
 * reference fjnunes and hortovanyi and udacity;
 * output results extract cpp file origin file
 */
...
namespace gazebo
{
    ...
    void ArmPlugin::onCollisionMsg(ConstContactsPtr &contacts)
    {
        ...
        for (unsigned int i = 0; i < contacts->contact_size(); ++i)
        {
          ...
          //--------------------------------- objective 1--------------------------------start
          // set true when test objective 1
          if (true)
          {
          bool collisionCheck1 = ((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0) 
                || (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_ARM) == 0)) ? true : false;
            ...
            {
              // Give penalty for un-wanted collisions
        ...
        //--------------------objective 2-------------------------------------------------------start
          if (false)
          {
            // define collisioncheck2 for objective2
          bool collisionCheck2 = ((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0) &&
               (strcmp(contacts->contact(i).collision2().c_str(), COLLISION_POINT) == 0)) ? true : false;
    ...
    void ArmPlugin::OnUpdate(const common::UpdateInfo& updateInfo)
    {
        ...
        if( hadNewState && !newReward )
        {
            ...
            {
                // for interim reward based on the distance to the object
                const float distGoal = BoxDistance(gripBBox, propBBox); // compute the distance from propbox to the gripbbox
                
                if(DEBUG){printf("distance('%s', '%s') = %f\n", gripper->GetName().c_str(), prop->model->GetName().c_str(), distGoal);}
                
                // assign reward with the ratial of the distGoal using moving average distance.
                if( episodeFrames > 1 )
                {
